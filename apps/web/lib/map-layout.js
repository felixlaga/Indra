/** @typedef {import('./types').ResearchMapNode} ResearchMapNode */
/** @typedef {import('./types').ResearchMapCluster} ResearchMapCluster */
/** @typedef {import('./types').ResearchMapEdge} ResearchMapEdge */
/** @typedef {ResearchMapNode & {x: number, y: number, radius: number}} PositionedNode */

/**
 * @param {ResearchMapNode[]} nodes
 * @param {ResearchMapCluster[]} clusters
 * @param {number} [width]
 * @param {number} [height]
...
export function layoutResearchMap(nodes, clusters, width = 1040, height = 560) {
  ...
  return nodes.map((node) => {
    ...
    const yearRatio = typeof node.year === "number"
      ? (node.year - minYear) / yearSpan
      : 0.5;
    ...
    const radius = Math.max(
      7,
      Math.min(18, 7 + Math.log1p(node.citation_count ?? 0) * 1.7),
    );
...
/**
 * @param {ResearchMapEdge} edge
 * @param {PositionedNode[]} positionedNodes
